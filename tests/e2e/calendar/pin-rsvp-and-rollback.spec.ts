...
test.describe('Calendar - PIN, RSVP e Rollback Visual', () => {
  test('13.1 - PIN inválido não exclui evento e mantém item na UI', async ({
    ...
    await createCalendarEventViaApi(ownerToken, buildDefaultEventPayload(title));
  ...
  test('13.2 - Cancelar modal de exclusão não altera evento', async ({ page }) => {
    ...
    await page.locator('button:has-text("Excluir")').click();
    await expect(page.locator('text=Excluir Evento')).toBeVisible({ timeout: 10_000 });
    await page.locator('[role="dialog"] button:has-text("Cancelar")').click();
  ...
  test('13.3 - RSVP recusar e talvez exibem feedback correto', async ({ page }) => {
  ...
  test('13.5 - Erro ao editar mantém título anterior no calendário', async ({ page }) => {
    ...
    await expect(page.locator(`.fc-event:has-text("${oldTitle}")`).first()).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.locator(`.fc-event:has-text("${newTitle}")`)).toHaveCount(0);
  });
});
