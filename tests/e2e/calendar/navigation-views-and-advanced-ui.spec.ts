...
test.beforeAll(async () => {
  ...
  const viewOnlyAuth = await getAuthenticatedToken(
    viewOnly.email,
    viewOnly.password
  );
...
test.describe('Calendar - P1 Interface (Navegação e UI Avançada)', () => {
  ...
  test('8.2 - Alternar visualizações Mês/Semana/Dia/Agenda', async ({
    ...
    await expect(
      page.locator('button.fc-timeGridWeek-button.fc-button-active')
    ).toBeVisible();

    await page.locator('button.fc-timeGridDay-button').click();
    await expect(
      page.locator('button.fc-timeGridDay-button.fc-button-active')
    ).toBeVisible();

    await page.locator('button.fc-listWeek-button').click();
    await expect(
      page.locator('button.fc-listWeek-button.fc-button-active')
    ).toBeVisible();

    await page.locator('button.fc-dayGridMonth-button').click();
    await expect(
      page.locator('button.fc-dayGridMonth-button.fc-button-active')
    ).toBeVisible();
  });
  ...
  test('8.4 - Criar evento com campos avançados de interface', async ({
    page,
  }) => {
    ...
    await page
      .locator('textarea[placeholder="Descrição do evento"]')
      .fill(description);
    ...
    await page
      .locator('[role="dialog"] button[aria-label="Selecionar cor Azul"]')
      .click();
  ...
  test('8.5 - Criar evento recorrente e exibir badge no detalhe', async ({
    page,
  }) => {
  ...
  test('8.6 - Cancelar create e edit sem persistir alterações', async ({
    page,
  }) => {
    ...
    await createCalendarEventViaApi(
      editorToken,
      buildDefaultEventPayload(originalTitle)
    );
