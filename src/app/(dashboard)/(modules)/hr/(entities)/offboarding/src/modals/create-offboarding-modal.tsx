...
import { useCreateOffboarding } from '../api/mutations';
import {
  ClipboardCheck,
  ListTodo,
  Plus,
...
export function CreateOffboardingModal({
  open,
  onOpenChange,
  onClose,
  ...
  const [title, setTitle] = useState('Checklist de Desligamento');
  ...
  const handleReset = useCallback(() => {
    setCurrentStep(1);
    setEmployeeId('');
    setTitle('Checklist de Desligamento');
  ...
  const handleSubmit = useCallback(() => {
    ...
    createOffboarding.mutate({
      employeeId,
      title: title.trim() || 'Checklist de Desligamento',
      items:
        validItems.length > 0
          ? validItems.map(item => ({
  ...
  }, [employeeId, title, draftItems, createOffboarding]);
  ...
  const steps: WizardStep[] = [
    {
      ...
      description:
        'Selecione o funcionário e defina o título do checklist de desligamento',
      icon: (
        <UserMinus className="h-16 w-16 text-rose-500 dark:text-rose-400" />
      ),
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: Desligamento Equipe TI"
            />
          </div>
        </div>
      ),
    },
    {
      ...
      description:
        'Adicione as tarefas de desligamento (deixe vazio para usar o padrão)',
      icon: (
        ...
        <div className="space-y-3 p-4 max-h-[320px] overflow-y-auto">
          ...
          {!hasValidItems && (
            <p className="text-xs text-muted-foreground text-center">
              Se nenhum item for preenchido, serão usados os 14 itens padrão de
              desligamento.
            </p>
          )}
        </div>
      ...
      footer: (
        <div className="flex justify-between w-full">
          ...
          <Button onClick={handleSubmit} disabled={createOffboarding.isPending}>
            <ClipboardCheck className="h-4 w-4 mr-1" />
            {createOffboarding.isPending ? 'Criando...' : 'Criar Checklist'}
          </Button>
        </div>
      ),
    },
