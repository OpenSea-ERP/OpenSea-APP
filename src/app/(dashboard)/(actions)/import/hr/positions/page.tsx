...
export default function ImportPositionsPage() {
  ...
  const entityDef = ENTITY_DEFINITIONS.positions;

  // Build fields - stable reference
  const enabledFields = useMemo(() => {
    const baseFields = getEntityFields('positions');
      ...
      <Card className="mb-4 border-amber-500/30 bg-amber-500/5">
        <CardContent className="flex items-center gap-4 py-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold">Importação de Cargos</h3>
            <p className="text-sm text-muted-foreground">
              Preencha a planilha com os dados dos cargos ou cole dados de outra
              planilha. Você também pode baixar o template Excel para
              preenchimento.
            </p>
          </div>
        </CardContent>
        ...
        <CardContent className="flex items-center justify-between py-3">
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="gap-1">
              ...
              {spreadsheet.filledRowCount === 1 ? 'cargo' : 'cargos'} para
              importar
            </Badge>
            {validationResult &&
