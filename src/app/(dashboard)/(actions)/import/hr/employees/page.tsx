...
export default function ImportEmployeesPage() {
  ...
  const entityDef = ENTITY_DEFINITIONS.employees;

  // Build fields - stable reference
  const enabledFields = useMemo(() => {
    const baseFields = getEntityFields('employees');
  ...
  const handleImportComplete = useCallback(
    (result: { importedRows: number }) => {
      toast.success(
        `Importação concluída! ${result.importedRows} funcionários importados.`
      );
    },
    []
  );
      ...
      <Card className="mb-4 border-teal-500/30 bg-teal-500/5">
        <CardContent className="flex items-center gap-4 py-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold">Importação de Funcionários</h3>
            <p className="text-sm text-muted-foreground">
              Preencha a planilha com os dados dos funcionários ou cole dados de
