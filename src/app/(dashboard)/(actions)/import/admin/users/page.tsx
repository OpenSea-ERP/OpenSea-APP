...
export default function ImportUsersPage() {
  ...
  const entityDef = ENTITY_DEFINITIONS.users;

  // Build fields - stable reference
  const enabledFields = useMemo(() => {
    const baseFields = getEntityFields('users');
      ...
      <Card className="mb-4 border-pink-500/30 bg-pink-500/5">
        <CardContent className="flex items-center gap-4 py-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold">Importação de Usuários</h3>
            <p className="text-sm text-muted-foreground">
              Preencha a planilha com os dados dos usuários ou cole dados de
        ...
        <CardContent className="flex items-center justify-between py-3">
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="gap-1">
              ...
              {spreadsheet.filledRowCount === 1 ? 'usuário' : 'usuários'} para
              importar
            </Badge>
            {validationResult &&
