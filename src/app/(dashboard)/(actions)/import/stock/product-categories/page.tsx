...
export default function ImportProductCategoriesPage() {
  ...
  const entityDef = ENTITY_DEFINITIONS['product-categories'];

  // Build fields - stable reference
  const enabledFields = useMemo(() => {
    const baseFields = getEntityFields('product-categories');
  ...
  const handleImportComplete = useCallback(
    (result: { importedRows: number }) => {
      toast.success(
        `Importação concluída! ${result.importedRows} categorias importadas.`
      );
    },
    []
  );
    ...
    <PageLayout backgroundVariant="none" maxWidth="full">
      ...
      <Card className="mb-4 border-yellow-500/30 bg-yellow-500/5">
        <CardContent className="flex items-center gap-4 py-4">
          <FolderTree className="w-8 h-8 text-yellow-500" />
          <div className="flex-1">
            <h3 className="font-semibold">Importação de Categorias</h3>
            <p className="text-sm text-muted-foreground">
              Preencha a planilha com os dados das categorias ou cole dados de
              outra planilha. Você também pode baixar o template Excel para
              preenchimento.
            </p>
          </div>
        </CardContent>
        ...
        <CardContent className="flex items-center justify-between py-3">
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="gap-1">
              ...
              {spreadsheet.filledRowCount === 1 ? 'categoria' : 'categorias'}{' '}
              para importar
            </Badge>
            {validationResult &&
